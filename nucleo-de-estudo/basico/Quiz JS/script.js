...
document.querySelector("input#button").addEventListener("click", gerar_quiz)
...
if (Points == 5){
    document.querySelector("div#ResultadoFinal").innerHTML = `<img src="./imagens/5-Netero.jpg" id="ImagemResultado"><p>Parabéns <strong>${NomeJogador}</strong>! A sua pontuação foi de 5 pontos, um acerto perfeito, digno do maior presidente da história da Associação Hunter, <strong>Isaac Netero</strong>.</p>`
    document.querySelector("div#ResultadoFinal").style.border="#f9db6d solid 10px"
}
else if (Points == 4){
    document.querySelector("div#ResultadoFinal").innerHTML = `<img src="./imagens/4-Hisoka.jpg" id="ImagemResultado"><p>Parabéns <strong>${NomeJogador}</strong>! A sua pontuação foi de 4 pontos, sem dúvidas um mestre do Nen. No universo de HxH você estaria próximo do nível de personagens monstruosos como <strong>Hisoka Morow</strong>.</p>`
    document.querySelector("div#ResultadoFinal").style.border="#C92DF7 solid 10px"
}
else if (Points == 3){
    document.querySelector("div#ResultadoFinal").innerHTML = `<img src="./imagens/3-Kite.jpg" id="ImagemResultado"><p>Parabéns <strong>${NomeJogador}</strong>! A sua pontuação foi de 3 pontos, seus conhecimentos já são acima da média, mas sua capacidade de crescimento é infinita ainda. Personagens como <strong>Kite</strong> representam suas habilidades.</p>`
    document.querySelector("div#ResultadoFinal").style.border="#4188EB solid 10px"
}
else if (Points == 2){
    document.querySelector("div#ResultadoFinal").innerHTML = `<img src="./imagens/2-Gon.jpg" id="ImagemResultado"><p>É...Parabéns <strong>${NomeJogador}</strong>. A sua pontuação foi de 2 pontos, você tem conhecimento básico do universo HxH. Mas pense pelo lado positivo, assim como <strong>Gon Freecss</strong>, você ainda tem todo um continente de materiais para aprender e poder crescer até o topo!</p> `
    document.querySelector("div#ResultadoFinal").style.border="#14FC40 solid 10px"
}
else if (Points == 1){
    document.querySelector("div#ResultadoFinal").innerHTML = `<img src="./imagens/1-Zushi.jpg" id="ImagemResultado"><p><strong>${NomeJogador}</strong>, você fez 1 ponto apenas. Mas não se envergonhe, assim como <strong>Zushi</strong>, se você continuar treinando/lendo poderá um dia se tornar o maior conhecedor e retornar o teste, acertando as 5 questões!</p> `
    document.querySelector("div#ResultadoFinal").style.border="#E3E0DC solid 10px"
}
else if (Points == 0){
    document.querySelector("div#ResultadoFinal").innerHTML = `<img src="./imagens/0-Tompa.png" id="ImagemResultado"><p>Caramba <strong>${NomeJogador}</strong>, 0 pontos? Provavelmente você não viu nada dessa obra prima chamada HunterXHunter, que coisa feia. Volte quando você conseguir identificar esse personagem da foto acima e você será uma pessoa mais iluminada.</p> `
